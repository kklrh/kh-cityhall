...
local function getJobs()
    QBCore.Functions.TriggerCallback('qb-cityhall:server:receiveJobs', function(result)
        SendReactMessage('setJobs', {jobs = result})
    end)
end
...
RegisterNUICallback('applyJob', function(job, cb)
    ...
    if inRangeCityhall then
        print(json.encode(job), closestCityhall)
        print(job.selectedJobId, json.encode(job), closestCityhall)
        print(json.encode(job), closestCityhall, Config.Cityhalls[closestCityhall].coords)
        print(job.selectedJobId, json.encode(job), closestCityhall, Config.Cityhalls[closestCityhall].coords)
        TriggerServerEvent('qb-cityhall:server:ApplyJob', job.selectedJobId, Config.Cityhalls[closestCityhall].coords)
        setCityhallPageState(false, false)
    else
        QBCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
