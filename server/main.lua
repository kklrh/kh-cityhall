...
RegisterNetEvent('qb-cityhall:server:requestId', function(item, hall)
    ...
    elseif item == "weaponlicense" then
        ...
        info.lastname = Player.PlayerData.charinfo.lastname
        info.birthdate = Player.PlayerData.charinfo.birthdate
    elseif item == "hunt" then
        info.firstname = Player.PlayerData.charinfo.firstname
        info.lastname = Player.PlayerData.charinfo.lastname
        info.birthdate = Player.PlayerData.charinfo.birthdate
        local licenses = {["hunt"] = true, ["driver"] = Player.PlayerData.metadata["licences"].driver, ['weapon'] = Player.PlayerData.metadata["licences"].weapon, ["business"] = Player.PlayerData.metadata["licences"].business}
...
RegisterNetEvent('qb-cityhall:server:sendDriverTest', function(instructors)
    ...
    for i = 1, #instructors do
        ...
        if SchoolPlayer then
            ...
            TriggerEvent("qb-phone:server:sendNewMail", src, mailData)
        end
    end
